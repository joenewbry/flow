...
class SimpleChromaStore {
  ...
  async initialize() {
    try {
      ...
      console.log(`Successfully initialized collection "${this.collectionName}"`);
      return this;
    } catch (error) {
  ...
  async add(item) {
    try {
      ...
      console.error(`Failed to add item with ID ${item.id}:`, error);
      throw error;
    }
  }
  ...
  async addBatch(items) {
    try {
      ...
      console.log(`Successfully added batch of ${items.length} items`);
    } catch (error) {
      console.error(`Failed to add batch of ${items.length} items:`, error);
      throw error;
    }
  }
  ...
  async query(queryOptions) {
    try {
      const results = await this.collection.query({
        queryTexts: [queryOptions.query],
        nResults: queryOptions.nResults || 10,
...
async function createSimpleChromaStore(collectionName = 'screen_history', chromaServerUrl = 'http://localhost:8000') {
  try {
    ...
    const client = new ChromaClient({ path: chromaServerUrl });
    
    const store = new SimpleChromaStore(client, collectionName);
    await store.initialize();
    
    return store;
  } catch (error) {
    console.error('Failed to create ChromaDB store:', error);
    throw error;
  }
}
